		...
		if (FLAG_RB) {
			...
			if (FLAG_CUR == 5) {
				...
				$.post(url, {
					...
					w : $("#w").attr("value"),
					h : $("#h").attr("value"),
					imageurl : $("#imageurl").attr("value"),
					realurl : $("#realurl").attr("value"),
					email : $("#email").attr("value"),
					province : $("#p").find("option:selected").text(),
					city : $("#city").find("option:selected").text(),
					Date_Year : $("#Date_Year").find("option:selected").text(),
					birthday_m : $("#birthday_m").find("option:selected").text(),
					gender : $("#g").find("option:selected").attr("value"),
					role : $("input[name='role']:checked").val(),
					aims : aims,
					intro : $("#ii").val(),
					department : $("#department").attr("value"),
					post : $("#post").attr("value"),
					school : $("#school").attr("value"),
					major : $("#major").attr("value"),
					weibo : $("#weibo").attr("value"),
					...
					if (data.state == 'ok') {
						 window.location.href=BASE_URL + "user_space";
					} else {
						alert("error");
					}
				}, "json");
