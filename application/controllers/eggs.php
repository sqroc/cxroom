...
class Eggs extends CI_Controller {
	...
	public function attentioneggs() {
		...
		//分页配置结束
		$data['ideas'] = $this -> Projects_model -> showeggsByLimitByAttention($this -> uri -> segment(4, 0), $config['per_page']);
		...
		$data = array_merge($data, $this -> Common_model -> global_data());
		$this -> load -> view('eggs/header.php', $data);
		$this -> load -> view('eggs/eggs_attention.php', $data);
